...
<template>
  <UDashboardToolbar>
    <template #left>
      <UInput ref="input" v-model="query" icon="i-heroicons-funnel" autocomplete="off" :placeholder="$t('table.search')"
    ...
    <template #right>
      ...
      <UTooltip text="列表项">
        <USelectMenu v-model="showColumns" :options="columns.filter(i => i.label)" multiple placeholder="Columns"
  ...
  <UTable v-model="selected" v-model:sort="sort" :loading="loading" sort-mode="manual" :rows="list"
    :page-total="list.length" :columns="selectedColumns" v-bind="attrs" class="w-full"
    :ui="{ divide: 'divide-gray-200 dark:divide-gray-800' }"
    :empty-state="{ icon: 'i-heroicons-circle-stack-20-solid', label: 'No items.' }">
    <template #empty-state>
      <div v-if="error" class="flex flex-col items-center justify-center gap-3 py-10">
        <UIcon name="i-solar-danger-triangle-bold-duotone"
    ...
    <template v-if="menus" #opt-actions-data="{ row }">
      <div class="flex justify-end items-center">
        <div class="lg:block hidden">
          <ULink v-for="(item, i) in menus(row).flat().filter(menu => menu?.show)" :key="i" :to="item.to">
            <UButton variant="ghost" @click="item.click">
              {{ item.label }}
            </UButton>
          </ULink>
        </div>
  ...
  <nav class="flex flex-col items-center justify-between p-4 md:flex-row md:items-center space-y-3 md:space-y-0"
