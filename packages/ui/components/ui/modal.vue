...
const contentClass = {
  center: 'absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 max-h-screen',
...
<template>
  ...
  >
    <div :class="[cardClass[placementRef], fullScreen ? 'w-screen h-screen ' : 'md:py-5']">
      <Card class="h-full w-[400px]" :ui="{ ring: '', divide: description ? 'divide-y' : 'divide-y-0' }">
        <CardHeader v-if="!blank" class="flex-row justify-between">
          <div class=" space-y-1">
            <CardTitle>
              {{ title }}
            </CardTitle>
          ...
          <Button v-if="!hiddenClose" size="icon" variant="ghost" @pointerup="emit('update:modelValue', false)">
            <Iconify icon="heroicons:x-mark-20-solid" class="w-4 h-4" />
        ...
        <CardFooter v-if="!blank" class="flex justify-end pt-6">
          <slot name="footer" />
        </CardFooter>
      </Card>
    </div>
